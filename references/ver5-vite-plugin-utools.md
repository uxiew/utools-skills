...
Source snapshot: npm README rechecked 2026-06-09; npm registry latest rechecked 2026-06-07: `@ver5/vite-plugin-utools@0.4.1` published 2026-04-16. Source links: https://www.npmjs.com/package/@ver5/vite-plugin-utools, https://registry.npmjs.org/%40ver5%2Fvite-plugin-utools, https://github.com/uxiew/vite-plugin-utools
...
## README-aligned hard rules

- Follow the package README as the source of truth for project shape: `src/main.ts` is the Vite UI entry and `utools/preload.ts` is the preload entry.
- Install `@ver5/vite-plugin-utools` as a dev dependency and configure it in `vite.config.ts` with `configFile`.
- Author preload in TypeScript only: source `utools/plugin.json` must use `"preload": "preload.ts"` when the manifest lives in `utools/`.
- Do not hand-author `utools/preload.js`. `dist/preload.js` is generated by the plugin during `vite build`.
- Expose runtime services by named exports from `utools/preload.ts`. Named exports mount to `window[name]` where `name` is the plugin option; default is `window.preload`.
- Use generated mocks only for normal-browser UI preview and contract checks. Real uTools behavior still requires the generated output folder in uTools Developer Tools.
...
- Prefer named exports for all page-facing services.
...
- If a legacy reference shows `window.preload = { readFile() {} }`, translate it to `export function readFile(...) { ... }` in `utools/preload.ts`; do not copy the manual assignment into source.
...
Fix mock issues at the plugin/mock layer, not by sprinkling `undefined` checks through business UI. Custom mock returns should match the real API contract closely enough for normal UI flows. Treat browser mock as a lightweight UI preview, not as a substitute for uTools Developer Tools.
...
- `pnpm vite`: starts the UI dev server, builds preload, and writes generated `plugin.json` pointing at the dev server. In Developer Tools, point to the generated output `plugin.json`, not the source `utools/plugin.json`.
